...
//does the above function for all lines in list
bool noIntersects(list<vertex*> vertices,vertex *lastVertex, vertex *newVertex)
{
	if (vertices.size() < 3)
		return true;

	struct vertex *headChecker = vertices.front();
	struct vertex *anotherLineVertex1=vertices.front();
	struct vertex *anotherLineVertex2=vertices.front()->next;
	while (anotherLineVertex2!=NULL)
	{
		if(intersect(lastVertex, newVertex, anotherLineVertex1, anotherLineVertex2) || 
		   intersect(newVertex, headChecker,anotherLineVertex1, anotherLineVertex2))	
...
//a check for if a line exists in a polygon
int numIntersects4Interior(list<vertex*> vertices,GLfloat vertexX, GLfloat vertexY, int max_x )
{
	int county = 0;

	struct vertex *midpoint = new struct vertex;
	midpoint->x = vertexX; midpoint->y = vertexY; midpoint->z = 0.0; midpoint->w = 1.0; midpoint->next = NULL;

	struct vertex *boundaryPoint = new struct vertex;
	midpoint->x = max_x; midpoint->y = vertexY; midpoint->z = 0.0; midpoint->w = 1.0; midpoint->next = NULL;
...
//checks if proposed line will exists in side polygon by checking how many times it intersects as it goes to the edge of the window
bool inThatThang(list<vertex*> vertices, vertex *p1, vertex *p2)
{
	...
	int crossy = numIntersects4Interior(vertices, middyBoyX, middyBoyY, INT_MAX);
