...
//new generic c++ list tessy-------------------------------------------------------------------------------------------------------------
#ifndef OLD

#include <list>
//to draw a filled, untesselated polygon
void makeThePolygonSucka(list<vertex*> vertices)
{//loop through list of points and feed it to the glBegin
...
//clear the screen
void whiteItOut(list<vertex*> vertices)
{
	glColor3f(1.0, 1.0, 1.0);
	makeThePolygonSucka(vertices);
	glColor3f(1.0, 0.0, 0.0);
}
//to connect the dots in the order they were added
void lineEmUpSucka(list<vertex*> vertices)
{
	//kick you out if you try to connect just 1 point
	if (vertices.front()==NULL || vertices.front()->next==NULL)
	{
		cout << "can't draw lines with fewer than 2 points" << endl;
		exit(0);
	}
	vertex *firstPoint = vertices.front();
	vertex *secondPoint = firstPoint->next;
	//loop through list and connect each point one by one in the order it was put in list
	while (secondPoint!=NULL)
	{
		glBegin(GL_LINES);
...
//sexy recursive tesselator function
void commenceTesselation(list<vertex*> vertices, list<triangle> triangles, struct vertex *p1, struct vertex *p2, struct vertex *p3)
{
...
{//starts tesselation
	...
	if (vertices.size() < 3)//less than 3 points, do nothing
		return;
	if (vertices.size()==3)//equal to 3 points, already a triangle; add it to vector and leave
	{
		struct triangle TDawg = {p1,p2,p3};
		triangles.push_back(TDawg);
		return;
	...
	else//4 points or more
	{
		...
		while (vertices.size() >= 3)//tessy loop
		{
			if (vertices.size()==3)//when the list is deleted down to 3 vertices, add that triangle to vector and leave
			{
				p1=vertices.front(); p2=p1->next; p3=p2->next;
				struct triangle TDawg = {p1, p2, p3};
				triangles.push_back(TDawg);

				break;
			}
			if (vertices.size() == 4 && crossProduct(p1,p2,p3)>0.0)//if 4 sided and clockwise
			{
				vertex *p4 = p3->next;
			...
			else if (vertices.size() == 4 && crossProduct(p1,p2,p3)<0.0)//if 4 sided and counterclockwise
			{
				vertex *p4 = p3->next;
